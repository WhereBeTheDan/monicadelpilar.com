...
function ScrollspyElement(_opts) {
	/**
	...
	function notAtBottom() {
		if (useCSS) {
			el.classList.remove('is-atBottom');	
		}
	...
	function addCSSPosition() {
		if (useCSS) {
			if (el.classList.contains('has-positionTop')) {
				el.classList.remove('has-positionTop');
			}
			if (el.classList.contains('has-positionBottom')) {
				el.classList.remove('has-positionBottom');
			}
	...
	function removeCSSDirection() {
		if (useCSS) {
			if (el.classList.contains('has-directionFromTop')) {
				el.classList.remove('has-directionFromTop');
			}
			if (el.classList.contains('has-directionFromBottom')) {
				el.classList.remove('has-directionFromBottom');
			}
		}
	}
	...
	function destroy() {
		if (useCSS) {
			...
			if (el.classList.contains('is-atTop')) {
				el.classList.remove('is-atTop');
			}
			if (el.classList.contains('is-atBottom')) {
				el.classList.remove('is-atBottom');
			}
...
function Scrollspy(_opts) {
	...
	function loopCollection(){
		 ...
		 */
			 ...
			 */
			temp = collection[collection.length - 1];
			if ( temp.getVisibility() > 0 ) {
				current = temp;
			}
	...
	function add(_HTMLElement, _opts) {
		...
		var scrollspyElement = new ScrollspyElement({
			el:_HTMLElement, 
			useCSS: useCSS,
	 ...
	 */
	function remove( scrollspyElement ) {
		Iterator.remove( scrollspyElement, collection );
		scrollspyElement.destroy();
	}
